...
use serde::de::{Error as DeError, Deserialize as DeDeserialize, Deserializer};

#[derive(Debug, Deserialize)]
pub struct PlayurlResp { pub code: i32, pub data: PlayurlData }
#[derive(Debug, Deserialize)]
pub struct PlayurlData { pub dash: Option<PlayurlDash> }
...
struct PlayVideoWire {
    ...
    #[serde(default)] codecid: Option<i32>,
    #[serde(default)] width: Option<u32>,
    #[serde(default)] height: Option<u32>,
    #[serde(default)] bandwidth: Option<u64>,
}
...
struct PlayAudioWire {
    ...
    backup_url: Option<Vec<String>>,
    #[serde(default)] bandwidth: Option<u64>,
    #[serde(default)] codecs: Option<String>,
}
...
pub struct Dolby { #[serde(default)] pub audio: Option<Vec<PlayAudio>> }
#[derive(Debug, Deserialize, Clone)]
pub struct Flac { pub audio: PlayAudio }
#[derive(Debug, Deserialize, Clone)]
pub struct PlayurlDash { #
    ...
    #[serde(default)] pub dolby: Option<Dolby>, #[serde(default)] pub flac: Option<Flac> }

pub async fn fetch_dash_ugc(client: &Client, aid: u64, cid: u64, use_wbi: bool) -> Result<PlayurlDash> {
    ...
    let v: PlayurlResp = client
        ...
        .json()
        .await?;
    eprintln!("DEBUG UGC Response code: {}", v.code);
    if v.code != 0 { return Err(anyhow!("获取播放地址失败 code={}", v.code)); }
    let mut dash = v.data.dash.ok_or_else(|| anyhow!("没有 dash 返回"))?;
    if let Some(d) = &dash.dolby { if let Some(list) = &d.audio { for a in list { dash.audio.push(a.clone()); } } }
    if let Some(f) = &dash.flac { dash.audio.push(f.audio.clone()); }
    Ok(dash)
}

pub async fn fetch_dash_pgc(client: &Client, ep_id: u64, season_id: u64, use_wbi: bool) -> Result<PlayurlDash> {
    let base_url = "https://api.bilibili.com/pgc/player/web/v2/playurl";

    // 参考 BiliTools: 登录用户使用 qn=127, fnval=4048; 未登录用户使用 qn=64, fnval=16
    let (qn, fnval, fourk) = if use_wbi { ("127", "4048", "1") } else { ("64", "16", "0") };
    ...
    let dash_v = v.get("data").and_then(|d| d.get("dash")).or_else(|| v.get("result").and_then(|r| r.get("dash"))).or_else(|| v.get("result").and_then(|r| r.get("video_info")).and_then(|vi| vi.get("dash"))).ok_or_else(|| anyhow!("PGC 未返回 dash"))?;
